...
public class WearableItemLayer<E extends LivingEntity, T extends LivingCap<E>> extends Layer<E, T>
{
	...
	private void renderArmor(MatrixStack matStack, IRenderTypeBuffer buf, int packedLight, boolean hasEffect, ClientModel model, float r, float g, float b, ResourceLocation armorResource, PublicMatrix4f[] poses)
	{
		RenderType rt = ModRenderTypes.getAnimatedArmorModel(armorResource);
		IVertexBuilder vertexConsumer = ModRenderTypes.getArmorVertexBuilder(buf, rt, hasEffect);
		model.draw(matStack, vertexConsumer, packedLight, r, g, b, 1.0F, poses);
	}
	
	@Override
	public void renderLayer(T entityCap, MatrixStack poseStack, IRenderTypeBuffer buffer, int packedLightIn, PublicMatrix4f[] poses, float partialTicks)
	{
		...
		poseStack.pushPose();
		if(this.slot == EquipmentSlotType.HEAD && entity instanceof ZombieVillagerEntity)
		{
