...
public class ResizeTextButton extends Button
{
	...
	public void renderButton(MatrixStack matrixstack, int p_230431_2_, int p_230431_3_, float p_230431_4_)
	{
	    ...
	    drawCenteredString(matrixstack, fontrenderer, this.getMessage(), Math.round((this.x + this.width / 2) / scale), Math.round((this.y + (this.height - 8) / 2) / scale), j | MathHelper.ceil(this.alpha * 255.0F) << 24);
		matrixstack.popPose();
