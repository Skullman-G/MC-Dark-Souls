...
public class BonfireScreen extends Screen
{
	...
	protected void init()
	{
		...
		this.estusHealIcon = this.addRenderableWidget(new ImageButton(this.width / 2 - 20 - (16 / 2), this.height / 2 - 90 - (16 / 2), 16, 16, 0, 0, 0, new ResourceLocation(DarkSouls.MOD_ID, "textures/items/undead_bone_shard.png"), 16, 16, null, tooltip, null));
		...
		this.estusVolumeIcon = this.addRenderableWidget(new ImageButton(this.width / 2 + 10 - (16 / 2), this.height / 2 - 89 - (16 / 2), 16, 16, 0, 0, 0, new ResourceLocation(DarkSouls.MOD_ID, "textures/items/estus_flask_full.png"), 16, 16, null, tooltip, null));
		this.estusVolumeIcon.active = false;
		
		this.addRenderableWidget(new Button(this.width / 2 - (this.buttonWidth / 2), yPos + (yRank++ * (this.buttonHeight + 5)), this.buttonWidth, this.buttonHeight, new TranslatableComponent("gui.darksouls.teleport"), (button) ->
		{
		...
		this.reverseHollowingButton = this.addRenderableWidget(new Button(this.width / 2 - (this.buttonWidth / 2), yPos + (yRank++ * (this.buttonHeight + 5)), this.buttonWidth, this.buttonHeight, new TranslatableComponent("gui.darksouls.reverse_hollowing_button"), (button) ->
		{
		...
		this.kindleButton = this.addRenderableWidget(new Button(this.width / 2 - (this.buttonWidth / 2), yPos + (yRank++ * (this.buttonHeight + 5)), this.buttonWidth, this.buttonHeight, new TranslatableComponent("gui.darksouls.kindle"), (button) ->
		{
	         this.kindle();
	    }, tooltip));
		this.kindleButton.active = this.playerCap.isHuman() && this.playerCap.hasEnoughHumanity(1) && this.bonfiretileentity.canKindle();
		this.addRenderableWidget(new Button(this.width / 2 - (this.buttonWidth / 2), yPos + (yRank++ * (this.buttonHeight + 5)), this.buttonWidth, this.buttonHeight, new TranslatableComponent("gui.darksouls.attunements"), (button) ->
		{
	         ModNetworkManager.sendToServer(new CTSOpenAttunementScreen());
	    }));
		this.addRenderableWidget(new Button(this.width / 2 - (this.buttonWidth / 2), yPos + (yRank++ * (this.buttonHeight + 5)), this.buttonWidth, this.buttonHeight, new TranslatableComponent("gui.darksouls.leave_button"), (button) ->
		{
	         super.onClose();
	    }));
	}
