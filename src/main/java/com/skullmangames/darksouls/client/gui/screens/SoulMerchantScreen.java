...
@OnlyIn(Dist.CLIENT)
public class SoulMerchantScreen extends ContainerScreen<SoulMerchantMenu>
{
	...
	public SoulMerchantScreen(SoulMerchantMenu menu, PlayerInventory inventory, ITextComponent component)
	{
	...
	protected void init()
	{
		...
		for (int l = 0; l < 7; ++l)
		{
			this.tradeOfferButtons[l] = this
					.addButton(new TradeOfferButton(i + 5, k, l, (button) ->
					{
	...
	protected void renderBg(MatrixStack poseStack, float partialTicks, int mouseX, int mouseY)
	{
		...
		int i = (this.width - this.imageWidth) / 2;
		int j = (this.height - this.imageHeight) / 2;
		blit(poseStack, i, j, this.getBlitOffset(), 0.0F, 0.0F, this.imageWidth, this.imageHeight, 256, 512);
	...
	private void renderScroller(MatrixStack poseStack, int mouseX, int mouseY, SoulMerchantOffers offers)
	{
		...
		if (i > 1)
		{
			...
			blit(poseStack, mouseX + 94, mouseY + 18 + i1, this.getBlitOffset(), 0.0F, 199.0F, 6, 27, 256, 512);
		}
		else
		{
			blit(poseStack, mouseX + 94, mouseY + 18, this.getBlitOffset(), 6.0F, 199.0F, 6, 27, 256, 512);
		}
	...
	public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTicks)
	{
		...
		ITextComponent souls = new StringTextComponent(String.valueOf(this.playerCap.getSouls()));
	...
	private void renderButtonArrows(MatrixStack poseStack, SoulMerchantOffer offer, int x, int y)
	{
		RenderSystem.enableBlend();
		minecraft.getTextureManager().bind(TEXTURE_LOCATION);
		blit(poseStack, x + 5 + 35 + 20, y + 3, this.getBlitOffset(), 15.0F, 171.0F, 10, 9, 256, 512);
	...
	public boolean mouseScrolled(double p_99127_, double p_99128_, double p_99129_)
	{
		...
		{
			int j = i - 7;
			this.scrollOff = MathHelper.clamp((int) ((double) this.scrollOff - p_99129_), 0, j);
		}
	...
	private class TradeOfferButton extends Button
	{
		...
		public TradeOfferButton(int x, int y, int index, Button.IPressable onPress)
		{
