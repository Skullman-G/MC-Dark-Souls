...
public class Collider
{
	 ...
	 * @param center Center of the collider
	 */
	public Collider(AABB entityCallAABB, float pos1_x, float pos1_y, float pos1_z, float pos2_x, float pos2_y,
			float pos2_z, float norm1_x, float norm1_y, float norm1_z, float norm2_x, float norm2_y, float norm2_z,
	...
	public Collider(AABB aabb)
	{
		...
		worldCenter = new Vector3f(-((float) aabb.minX + xSize), (float) aabb.minY + ySize,
				-((float) aabb.minZ + zSize));
		rotatedVertex = new Vector3f[4];
		rotatedNormal = new Vector3f[3];
	...
	public Collider getScaledCollider(float scale)
	{
		...
		return new Collider(pos.x() * scale, pos.y() * scale, pos.z() * scale, center.x() * scale, center.y() * scale,
				center.z() * scale);
	}
	...
	public boolean collidesWith(Collider opponent)
	{
	...
	private static boolean collisionDetection(Vector3f seperateAxis, Vector3f toOpponent, Collider box1, Collider box2)
	{
		...
		distance = Vector3fHelper.dot(seperateAxis, toOpponent) > 0 ? toOpponent
				: new Vector3f(-toOpponent.x(), -toOpponent.y(), -toOpponent.z());

		for (Vector3f vertexVector : box1.rotatedVertex)
		{
			Vector3f temp = Vector3fHelper.dot(seperateAxis, vertexVector) > 0 ? vertexVector
					: new Vector3f(-vertexVector.x(), -vertexVector.y(), -vertexVector.z());
		...
		if (getProjectedScale(seperateAxis, distance) >= getProjectedScale(seperateAxis, maxProj1)
				+ getProjectedScale(seperateAxis, maxProj2))
		{
			return false;
		}
		return true;
	...
	private static float getProjectedScale(Vector3f normal, Vector3f projecting)
	{
		...
		Vector3f projVec = new Vector3f(dot * normal.x() * normalScale, dot * normal.y() * normalScale,
				dot * normal.z() * normalScale);

		return (float) Math
				.sqrt((projVec.x() * projVec.x()) + (projVec.y() * projVec.y()) + (projVec.z() * projVec.z()));
	}
			...
			float prevElapsedTime, float elapsedTime, String jointName, float attackSpeed)
	{
		...
		if (pathIndex == -1)
		{
			transformMatrix = new PublicMatrix4f();
		} else
		{
			transformMatrix = Animator.getBindedJointTransformByIndex(entityCap.getAnimator().getPose(1.0F), armature,
					pathIndex);
		}

		PublicMatrix4f toWorldCoord = PublicMatrix4f.createTranslation(-(float) entityCap.getOriginalEntity().getX(),
				(float) entityCap.getOriginalEntity().getY(), -(float) entityCap.getOriginalEntity().getZ());
		transformMatrix.mulFront(toWorldCoord.mulBack(entityCap.getModelMatrix(1.0F)));
	...
	@OnlyIn(Dist.CLIENT)
	public void draw(PoseStack matrixStackIn, MultiBufferSource buffer, PublicMatrix4f pose, float partialTicks,
			boolean red)
	{
