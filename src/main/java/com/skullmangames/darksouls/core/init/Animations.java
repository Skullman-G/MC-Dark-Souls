	...
	public static final DeathAnimation BIPED_DEATH_BACKSTAB = new DeathAnimation(0.05F, "biped/death/backstab_thrust", (models) -> models.ENTITY_BIPED)
			.addProperty(StaticAnimationProperty.EVENTS, new Event[]
			{
					Event.create(0.44F, Side.CLIENT, (cap) ->
					{
						...
						Vector3d pos = cap.getOriginalEntity().position().add(Math.cos(yRot), y, Math.sin(yRot));
			...
			.addProperty(StaticAnimationProperty.EVENTS, new Event[]
					{
							...
							{
								List<Entity> targets = cap.getLevel().getEntities(null, new AxisAlignedBB(cap.getX() - 3.0F, cap.getY() - 3.0F, cap.getZ() - 3.0F, cap.getX() + 3.0F, cap.getY() + 3.0F, cap.getZ() + 3.0F));
			...
			.addProperty(StaticAnimationProperty.EVENTS, new Event[]
					{
						...
						{
							if (cap.getOriginalEntity() instanceof ServerPlayerEntity)
							{
								BlockPos pos = ((ServerPlayerEntity)cap.getOriginalEntity()).getRespawnPosition();
			...
			.addProperty(StaticAnimationProperty.EVENTS, new Event[]
					{
						...
						{
							List<Entity> targets = cap.getLevel().getEntities(cap.getOriginalEntity(), new AxisAlignedBB(cap.getX() - 3.0F, cap.getY() - 3.0F, cap.getZ() - 3.0F, cap.getX() + 3.0F, cap.getY() + 3.0F, cap.getZ() + 3.0F));
							for (Entity target : targets)
							{
								if (target instanceof LivingEntity)
								{
									...
									if (targetCap != null)
									{
										DamageSourceExtended dmgSource = cap.getOriginalEntity() instanceof PlayerEntity ?
												ExtendedDamageSource.causePlayerDamage((PlayerEntity)cap.getOriginalEntity(), cap.getOriginalEntity().position(), StunType.FLY, 0, 0, 0, new Damage(DamageType.REGULAR, 0))
