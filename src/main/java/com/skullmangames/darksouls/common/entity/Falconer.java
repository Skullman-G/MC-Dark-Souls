...
public class Falconer extends ArmoredMob implements IRangedAttackMob
{
	public Falconer(EntityType<? extends Falconer> entitytype, World level)
	{
	...
	@Override
	public Item getEquipmentForSlot(int percentage, EquipmentSlotType slot)
	{
		if (percentage <= 75)
		{
			if (slot == EquipmentSlotType.MAINHAND) return ModItems.LONGSWORD.get();
			else if (slot == EquipmentSlotType.OFFHAND) return ModItems.GOLDEN_FALCON_SHIELD.get();
		}
		else
		{
			if (slot == EquipmentSlotType.MAINHAND) return Items.BOW;
		}
