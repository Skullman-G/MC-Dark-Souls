	...
	public static StaticAnimation getHumanoidHitAnimation(LivingCap<?> entityCap, ExtendedDamageSource dmgSource)
	{
		...
		else
		{
			...
			if (dmgSource.wasBlocked() && stunType != StunType.DISARMED)
			{
				...
				{
					if (entityCap.getOriginalEntity().getUsedItemHand() == Hand.MAIN_HAND) return Animations.BIPED_HIT_BLOCKED_FLY_RIGHT;
					return Animations.BIPED_HIT_BLOCKED_FLY_LEFT;
				}
				else
				{
					if (entityCap.getOriginalEntity().getUsedItemHand() == Hand.MAIN_HAND) return Animations.BIPED_HIT_BLOCKED_RIGHT;
