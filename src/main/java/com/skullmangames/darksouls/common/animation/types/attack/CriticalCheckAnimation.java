...
public class CriticalCheckAnimation extends AttackAnimation
{
	...
	protected boolean onDamageTarget(LivingCap<?> entityCap, Entity target)
	{
		...
		double yRotAttacker = Math.toRadians(MathUtils.toNormalRot(attacker.yRot));
		double dist = 1.0D;
		Vector3d dir = new Vector3d(Math.sin(yRotAttacker) * dist, 0, Math.cos(yRotAttacker) * dist);
		Vector3d pos = attacker.position().add(dir);
		target.setPos(pos.x, pos.y, pos.z);
		yRotAttacker = Math.toRadians(MathUtils.toNormalRot(attacker.yRot) - 90);
		dist = 0.5D;
		dir = new Vector3d(Math.sin(yRotAttacker) * dist, 0, Math.cos(yRotAttacker) * dist);
		pos = target.position().add(dir);
		target.setPos(pos.x, pos.y, pos.z);
		target.yRot = attacker.yRot;
		target.yRotO = attacker.yRot;
		ModNetworkManager.sendToAllPlayerTrackingThisEntity(new STCSetPos(target.position(), target.yRot, target.xRot, target.getId(), true), target);
		if (target instanceof ServerPlayerEntity)
		{
			ModNetworkManager.sendToPlayer(new STCSetPos(target.position(), target.yRot, target.xRot, target.getId()), (ServerPlayerEntity)target);
		}
	...
	@Override
	protected ExtendedDamageSource getDamageSourceExt(LivingCap<?> entityCap, Vector3d attackPos, Entity target, Phase phase, float amount)
	{
