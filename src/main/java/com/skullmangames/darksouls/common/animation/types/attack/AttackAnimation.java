...
public class AttackAnimation extends ActionAnimation
{
	...
	public void onUpdate(LivingCap<?> entityCap)
	{
		...
		if (state.shouldDetectCollision() || (prevState.getContactLevel() < 2 && state.getContactLevel() > 2))
		{
			...
			Vector3d prevColPos = collider.getWorldCenter();
	...
	public void onClientUpdate(LivingCap<?> entityCap)
	{
		...
		if (spawner != null)
		{
			...
			if (elapsedTime >= phase.contactEnd && prevElapsedTime - IngameConfig.A_TICK <= phase.contactEnd) spawner.spawnParticles((ClientWorld)entity.level, collider.getWorldCenter());
		}
